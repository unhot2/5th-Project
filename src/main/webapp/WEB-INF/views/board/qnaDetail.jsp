...
<section class="qnaDetailSection">
	...
	<div class="row">
		<table class="table">
          ...
          <th scope="col" style="width: 30%">
            <font size="3"><b>글번호 : ${qnaDetail.id}</b></font><br>
            <font size="3"><b>작성자 : ${qnaDetail.name }</b></font>
          </th>
					<th scope="col" style="width: 40%; text-align:center;">
            <font size="6"><b>${qnaDetail.title }</b></font>
          </th>
					<th scope="col" style="width: 30%" class="text-right">
             <font size="3"><b>조회수 : ${qnaDetail.hit }</b></font><br> 
            <font size="3"><b>작성일 : ${qnaDetail.savedate }</b></font>
          </th>
				</tr>
			</thead>
			<tbody>
    ...
    <c:choose>
		<c:when test="${userId ne null }">
			...
			<hr>
			<div>
				<form action="qnaReplyWrite?userid=${userId }&idgroup=${qnaDetail.idgroup}&id=${qnaDetail.id}" method="post">
            ...
            <div style="width:100%" align="center">
              <div style="float: left; width:90%">
                  <input class="form-control mt-4 mb-2" type="text" name="content" style="width:100%;">
              </div>
              <div>
                  <input class="btn btn-secondary mb-3" type="submit" value="작성" style="width:5%; margin-left:40px;">
              </div>
            </div>
          </div>
				</form>
	...
	<span>&nbsp;</span>
		<table class="table table-hover">
			<tr>
				<th class="text-center">작성자</th><th class="text-center">내용</th><th class="text-center">작성일</th>
			</tr>
	<c:forEach var="reply" items="${qnaReplyList }">
			<tr>
				<td class="text-center">${reply.userid }</td>
				<td class="text-center">${reply.content }</td>
				<td class="text-center"><fmt:formatDate value="${reply.rdate}" pattern="yyyy.MM.dd:hh.mm"/></td>
			</tr>
	</c:forEach>
		</table>
	</div>
